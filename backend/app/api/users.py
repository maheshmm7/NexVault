...
def seed_demo_data(
    ...
    """Idempotently seeds demo data. Rejected if user already has payment sources."""
    ...
    transactions = [
        Transaction(user_id=current_user.id, source_id=src_checking.id, category_id=cat_salary.id,
                    amount=45000.00, type="income",  notes="Monthly Salary",     timestamp=now - timedelta(days=20)),
        Transaction(user_id=current_user.id, source_id=src_checking.id, category_id=cat_housing.id,
                    amount=12000.00, type="expense", notes="Rent — May 2026",    timestamp=now - timedelta(days=18)),
        Transaction(user_id=current_user.id, source_id=src_card.id,     category_id=cat_food.id,
                    amount=1850.00,  type="expense", notes="BigBasket Grocery",  timestamp=now - timedelta(days=10)),
        Transaction(user_id=current_user.id, source_id=src_card.id,     category_id=cat_food.id,
                    amount=320.00,   type="expense", notes="Zomato",             timestamp=now - timedelta(days=6)),
        Transaction(user_id=current_user.id, source_id=src_card.id,     category_id=cat_shopping.id,
                    amount=4599.00,  type="expense", notes="Amazon — Headphones",timestamp=now - timedelta(days=3)),
        Transaction(user_id=current_user.id, source_id=src_wallet.id,   category_id=cat_food.id,
                    amount=210.00,   type="expense", notes="Swiggy",             timestamp=now - timedelta(days=1)),
    ]
    ...
    coupons = [
        Coupon(user_id=current_user.id, title="Swiggy 30% Off", code="SWIGGY30",
               category="Food", expiry_date=now + timedelta(days=10), status="active"),
        Coupon(user_id=current_user.id, title="Amazon Gift Card", code="AMZN-GIFT-50",
               pin="8821", category="Shopping", expiry_date=now + timedelta(days=30), status="active"),
        Coupon(user_id=current_user.id, title="Uber Eats ₹100 Off", code="EATS100",
               category="Food", expiry_date=now - timedelta(days=3), status="expired"),
    ]
    db.add_all(coupons)
